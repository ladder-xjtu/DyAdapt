...
class ElasticNet(nn.Module):
    """ElasticNet regularizaton on activations, e.g., adaptive kernels
    Args:
        alpha: A tuning parameter balancing between lasso and ridge
    ...
    def forward(self, predict):
        lasso = torch.sum(torch.abs(predict))
        ridge = torch.sum(torch.pow(predict, 2))
        loss = (1 - self.alpha) * lasso + self.alpha * ridge
        return loss