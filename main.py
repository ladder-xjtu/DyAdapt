...
from MTNetwork import DyAdaptNet
from dataset import create_folds, Dataset
from dice_loss import DiceLoss, ElasticNet

primary_dir = '/data/DATA/xuanang_datasets/prostate_bed/' # please modify the primary directory to your dataset folder
...
dir_models = '/data/Experiments/' + 'DyAdaptNet_trained_models/' # output folder storing trained model files and predicted masks.
...
def train_net(net,
    ...
    for fold_id in range(fold_num):
        ...
        val_loader = data.DataLoader(
            dataset=val_set, batch_size=36, shuffle=False, pin_memory=True, drop_last=False, num_workers=num_workers)
        ...
        test_loader = data.DataLoader(
            dataset=test_set, batch_size=12, shuffle=False, pin_memory=True, drop_last=False, num_workers=num_workers)
        ...
        criterion = DiceLoss()
        criterion_pb = DiceLoss()
        regularizer = ElasticNet(alpha=0.5)
        ...
        for epoch in range(epoch_num):
            ...
            for batch_id, batch in enumerate(train_loader):
                ...
                # forward propagation
                pd_oar_prob, pd_pb_prob, pd_oar_dprob, pd_pb_dprob, w_pb = net(imgs, dimgs)

                # compute loss
                loss_pb = criterion_pb(pd_pb_prob, gt_pb_mask)
                loss_oar = criterion(pd_oar_prob, gt_oar_mask)
                loss_doar = criterion(pd_oar_dprob, gt_oar_dmask)
                loss_dpb = criterion_pb(pd_pb_dprob, gt_pb_dmask)
                regularizer_pb = regularizer(w_pb)
                loss = weight_pb * loss_pb + weight_oar * loss_oar + weight_d * (loss_doar + loss_dpb) + weight_w * regularizer_pb#(regularizer_oar + regularizer_pb)
                train_loss += n * loss.item()
                train_sample_num += n

                print('Fold {0:d}/{1:d} --- Epoch {2:d}/{3:d} --- Progress {4:5.2f}% (+{5:d}) --- Loss: {6:.6f} ({7:.6f}/{8:.6f}/{9:.6f}/{10:.6f})'.format(
                    fold_id+1, fold_num, epoch+1, epoch_num, 100.0 * batch_id * batch_size / len(train_ids), n, loss.item(), loss_pb.item(), loss_oar.item(), loss_dpb.item(), loss_doar.item()))
...
if __name__ == '__main__':
    ...
    resample_size = [256, 256] # resample size of the input CT slices
    resample_spacing = [1.0, 1.0] # resample spacing (resolution) of the input CT slices
    ...
    num_workers = 20 # you can use multiple CPU threads to speed up data loading.
    ...
    model = DyAdaptNet(in_chns=adjacent_slice_num * 2 + 1, num_ot1=3, num_ot2=2, scale_factor=2,
                    with_res=True, with_bn=True, with_drop=False, drop_rate=0.5)
