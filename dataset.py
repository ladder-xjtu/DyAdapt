...
def create_folds(img_dir, dir_models, fold_num, exclude_case, slicewise=False):
    fold_file_name = os.path.dirname(dir_models[:-1]) + '/CV_{0:d}-fold.txt'.format(fold_num)
    folds = {}
...
class Dataset(data.Dataset):
    ...
    min_hu, max_hu, oar_labels, adjacent, is_training, buffered_in_memory, scale_factor=2):
        self.ids = ids
    ...
    def __getitem__(self, index):
        ...
        image['array_dsample'] = self.normalize(image['array_dsample'])
        ...
        oar_darray = np.zeros_like(oar_mask['array_dsample'])
        for i in range(len(self.oar_labels)):
            label = self.oar_labels[i]
            oar_array[oar_mask['array'] == label] = i + 1
            oar_darray[oar_mask['array_dsample'] == label] = i + 1
        oar_mask['array'] = oar_array
        oar_mask['array_dsample'] = oar_darray
        ...
        if self.adjacent > 0:
            ...
            stack_shape_dsample = np.array(image['array_dsample'].shape)
            stack_shape_dsample[0] = self.adjacent * 2 + 1
            img_stack_array_dsample = np.zeros(stack_shape_dsample, dtype=np.float32)
            img_stack_array_dsample[self.adjacent, :] = image['array_dsample']
            ...
            for offset in range(-self.adjacent, self.adjacent+1):                
                ...
                if offset != 0 and os.path.exists(adj_image_name):
                    ...
                    adj_image['array_dsample'] = self.normalize(adj_image['array_dsample'])
                    img_stack_array_dsample[self.adjacent + offset, :] = adj_image['array_dsample']
            
            image['array'] = img_stack_array
            image['array_dsample'] = img_stack_array_dsample

        image_tensor = torch.from_numpy(image['array'])
        image_tensor_dsample = torch.from_numpy(image['array_dsample'])
        ...
        pb_dtensor = self.make_one_hot(torch.from_numpy(pb_mask['array_dsample']), num_classes=2)
        oar_dtensor = self.make_one_hot(torch.from_numpy(oar_mask['array_dsample']), num_classes=3)

        output = {}
        output['data'] = image_tensor
        output['data_dsample'] = image_tensor_dsample
        output['pb_label'] = pb_tensor
        output['oar_label'] = oar_tensor
        output['pb_dlabel'] = pb_dtensor
        output['oar_dlabel'] = oar_dtensor
        output['filename'] = id
    ...
    def resample_image(self, image, pixeltype, resample_size, resample_spacing,
                       transform, linear_interpolate, dtype, scale_factor=2, input_img=False):
        ...
        resampler.Update()
        #oimage = resampler.GetOutput()
        image_array = itk.GetArrayFromImage(resampler.GetOutput())
        image_array = image_array[np.newaxis, :].astype(dtype)
        output['array'] = image_array

        # downsampled image
        if True:
            new_size_dsample = (int(resample_size[0] / scale_factor), int(resample_size[1] / scale_factor))
            #print(new_size_dsample)
            new_spacing_dsample = (resample_spacing[0] * scale_factor, resample_spacing[1] * scale_factor)
            new_origin_dsample = (
                origin[0] + size[0] * spacing[0] * 0.5 - new_size_dsample[0] * new_spacing_dsample[0] * 0.5,
                origin[1] + size[1] * spacing[1] * 0.5 - new_size_dsample[1] * new_spacing_dsample[1] * 0.5)

            output['size_dsample'] = np.array(new_size_dsample, dtype=int)
            output['spacing_dsample'] = np.array(new_spacing_dsample, dtype=float)
            output['origin_dsample'] = np.array(new_origin_dsample, dtype=float)

            resampler = itk.ResampleImageFilter[imagetype, imagetype].New()
            resampler.SetInput(image)
            resampler.SetSize(new_size_dsample)
            resampler.SetOutputSpacing(new_spacing_dsample)
            resampler.SetOutputOrigin(new_origin_dsample)
            resampler.SetTransform(transform)
            if linear_interpolate:
                resampler.SetInterpolator(itk.LinearInterpolateImageFunction[imagetype, itk.D].New())
            else:
            ...
            image_array = itk.GetArrayFromImage(resampler.GetOutput())
            image_array = image_array[np.newaxis, :].astype(dtype)
            output['array_dsample'] = image_array
